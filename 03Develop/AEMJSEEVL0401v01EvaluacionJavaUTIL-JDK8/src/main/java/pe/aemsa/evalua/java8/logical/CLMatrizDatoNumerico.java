...
public class CLMatrizDatoNumerico {
    ...
    public static void ordenar(double[] m) {
        ...
        while (s && (--numero_de_elementos > 0)) {
            ...
            for (i = 1; i <= numero_de_elementos; i++) // � el elemento (i-1) es mayor que el (i) ?
            {
                if (m[i - 1] > m[i]) {
    ...
    private static void qs(double[] m, int inf, int sup) {
        ...
        mitad = m[(izq + der) / 2];
        do {
            while (m[izq] < mitad && izq < sup) {
                izq++;
            }
            while (mitad < m[der] && der > inf) {
                der--;
            }
            if (izq <= der) {
                x = m[izq];
    ...
    private static void qs(String[] m, int inf, int sup) {
        ...
        mitad = m[(izq + der) / 2];
        do {
            while (m[izq].compareTo(mitad) < 0 && izq < sup) {
                izq++;
            }
            while (mitad.compareTo(m[der]) < 0 && der > inf) {
                der--;
            }
            if (izq <= der) {
                x = m[izq];
