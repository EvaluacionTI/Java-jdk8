...
// Si se produce un error durante la copia, se lanzar�
// el siguiente tipo de excepci�n:
public class CV08v01CopiarFichero {
    ...
    public static void copiar(String fuente, String destino)
        ...
        try {
            ...
            // padre existe y no est� protegido contra escritura
            {
            ...
            while (true) {
                ...
                if (nbytes == -1) {
                    break; // se lleg� al final del fichero
                }
                fDestino.write(buffer, 0, nbytes);
            }
        } // Cerrar cualquier flujo que est� abierto
        finally {
            try {
                if (fFuente != null) {
                    fFuente.close();
    ...
    // un directorio. El m�todo siguiente trata este caso.
    private static File directorioPadre(File f) {
        String nombreDir = f.getParent();
        if (nombreDir == null) // El m�todo getProperty con el par�metro "user.dir" devuelve
        // el directorio actual de trabajo.
        {
            return new File(System.getProperty("user.dir"));
        } else // Devolver el directorio padre del fichero
        {
    ...
    public static void main(String[] args) {
        // main debe recibir dos par�metros: el fichero fuente y
        // el destino.
        if (args.length != 2) {
